...
class esrtf_handler:
	...
	def search_old(self, mysql_conn, str_ex_words, str_in_words):
		...
		response = s.execute()
		return response
	def search_syno(self, mysql_conn, topic_in_words, topic_ex_words):
		...
		for i in range(len(list_in)):
			q_in = Q("query_string", query='\"%s\"' % list_in[i], fields=['title', 'content'])
			#print list_in[i]
			...
			if len(syno_list) == 0:
				...
				for j in range(len(sync_str_list)):
					p_in = Q("query_string", query='\"%s\"' % sync_str_list[j], fields=['title', 'content'])
					q_in = q_in | p_in
				rule.append(q_in)
		if len(topic_ex_words) > 0:
			list_ex = topic_ex_words.split(',')
			if len(list_ex) > 0:
				q_ex = Q("query_string", query='\"%s\"' % list_ex[0], fields=['title', 'content'])
				for k in range(1, len(list_ex)):
					p_ex = Q("query_string", query='\"%s\"' % list_ex[k], fields=['title', 'content'])
					q_ex = q_ex | p_ex
				rule.append(q_ex)
		query_all = rule[0]
		...
		s = Search(index='isearch6', doc_type=doc_type).using(self.es).query(query_all)
		response = s.execute()
		print 'response.hits.total: %d' % response.hits.total
		# for hit in response:
